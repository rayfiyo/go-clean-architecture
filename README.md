...
2. アプリケーション層 (Application Layer)
   ...
   - **ポート**: 「どんな型でもいいけど、このメソッドは必ず持ってね」という契約のまとまり
     - Go であれば interface の定義
     - ユースケース層（中側）が必要とする機能の抽象的な入り口
     - 例: Parser ポート
       ```go
       type Parser interface {
         ParseOne(src string) (domain.Command, error)
       }
       ```
       - これは「ParseOne というメソッドを持って、
         文字列(src)を domain.Command にして返せるものなら、どんな型でもいい」という契約。
       - ユースケースは この契約だけ知っていて、
         中身（WhitespaceParser なのか JSONParser なのか）は知らない。
       - 実際の実装は adapter 側が Parser を実装する。
3. インターフェース層 / アダプタ層 (Interface / Adapter Layer)
   - 役割: 外界（Web, CLI, DB, ファイルなど）とアプリケーション層をつなぐ「翻訳者」
   - 構成要素:
     - コントローラ / ハンドラ:
       HTTP や CLI からのリクエストを受けて、ユースケースを呼び出す
...
- それ、HTTP や DB の型名が登場する？ → adapter
- それ、このユースケース固有の段取り？ → usecase
- それ、再利用したい不変のルール？ → domain
- それ、外の世界（時刻/環境/ネットワーク）に触れる？
  - 抽象（interface）は usecase、具象は adapter
- それ、失敗時の扱い（再試行/ロールバック/中断）？ → usecase
